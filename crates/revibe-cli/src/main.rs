//! Revibe CLI - Mistral's open-source CLI coding assistant.
//!
...
async fn check_and_resolve_trusted_folder() -> Result<()> {
    use revibe_core::trusted_folders::TrustedFoldersManager;
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::from_default_env()
                .add_directive("revibe=info".parse().unwrap())
    ...
    // Load configuration
    let config = revibe_core::VibeConfig::load(args.agent.as_deref()).map_err(|e| {
        match e {
            revibe_core::error::VibeError::MissingPromptFile {
                system_prompt_id,
                prompt_dir,
            } => {
    ...
    let loaded_messages = if args.continue_session || args.resume.is_some() {
        ...
        let session_to_load = if args.continue_session {
            // Find latest session
            match revibe_core::session::SessionLogger::find_latest(&config.session_logging).await {
        ...
        if let Some(session_path) = session_to_load {
            match revibe_core::session::SessionLogger::load(&session_path).await {
                Ok(session_log) => {
    ...
    if args.prompt.is_some() {
        ...
        if !prompt.is_empty() {
            ...
            let programmatic_mode = revibe_core::modes::AgentMode::AutoApprove;
