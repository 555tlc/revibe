...
pub struct WelcomeBanner {
    ...
    /// Current animation frame for the R logo.
...
impl WelcomeBanner {
    ...
    pub fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        // Create the R logo with animated gradient colors
    ...
    /// Create the animated R logo with gradient colors and text information.
    fn create_animated_logo(&self) -> Vec<Line<'static>> {
        ...
        let max_text_width = [
            format!("Revibe v{}", env!("CARGO_PKG_VERSION")).len(),
        ...
        // Create the R logo with text information next to it (matching widgets.rs)
        vec![
            ...
            Line::from(vec![
                ...
                Span::raw(SPACE),
                Span::styled(
                    format!("{LOGO_TEXT_GAP}Revibe v{}", env!("CARGO_PKG_VERSION")),
                    Style::default().bold(),
                ),
                Span::raw(" ".repeat(
                    max_text_width - format!("Revibe v{}", env!("CARGO_PKG_VERSION")).len(),
                )),
            ]),
            ...
            // Line 4: left + diagonal start - "  ▇▇    ▇▇    " (2 + 2 + 4 + 2 + 4 = 14)
            Line::from(vec![
                Span::raw(SPACE),
                Span::styled(BLOCK, Style::default().fg(get_line_color(3))),
                Span::raw(SPACE),
                Span::raw(SPACE),
                Span::styled(BLOCK, Style::default().fg(get_line_color(3))),
                Span::raw(SPACE),
                Span::raw(SPACE),
                Span::raw(" ".repeat(max_text_width + LOGO_TEXT_GAP.len())), // Pad to match other lines
            ]),
            // Line 5: left + diagonal kick - "  ▇▇        ▇▇" (2 + 2 + 6 + 2 = 12)
            Line::from(vec![
                Span::raw(SPACE),
                ...
                Span::styled(BLOCK, Style::default().fg(get_line_color(4))),
                Span::styled(
                    format!("{LOGO_TEXT_GAP}{display_workdir}"),
                    Style::default().fg(colors::MUTED),
                ),
