...
use revibe_tools::manager::ToolManager;
use std::sync::Arc;
...
impl revibe_mcp::transport::Transport for MockTransport {
    async fn send(&self, _request: &str) -> revibe_mcp::error::McpResult<String> {
    ...
    async fn close(&self) -> revibe_mcp::error::McpResult<()> {
        Ok(())
    }
}
