...
/// Get the Revibe home directory.
///
/// Uses `REVIBE_HOME` environment variable if set, otherwise `~/.revibe`.
pub fn revibe_home() -> PathBuf {
    std::env::var("REVIBE_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            dirs::home_dir()
...
/// Global Revibe home directory.
pub static REVIBE_HOME: Lazy<PathBuf> = Lazy::new(revibe_home);

/// Configuration directory.
pub static CONFIG_DIR: Lazy<PathBuf> = Lazy::new(|| REVIBE_HOME.clone());
...
pub const LOCAL_CONFIG_DIR: &str = ".revibe";
