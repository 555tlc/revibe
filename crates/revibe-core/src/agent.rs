...
use revibe_llm::backend::{
    Backend, BackendFactory, CompletionOptions, ModelConfig as LlmModelConfig,
};
use revibe_llm::format::ApiToolFormatHandler;
use revibe_llm::{AvailableTool, LlmMessage, LlmUsage, Role, StrToolChoice, ToolCall, ToolChoice};
use revibe_mcp::client::McpClient;
use revibe_mcp::transport::{HttpTransport, StdioTransport};
use revibe_tools::ToolManager;
use revibe_tools::types::{PatternCheckResult, ToolPermission};
...
impl Agent {
    ...
    pub fn new(config: VibeConfig, mode: AgentMode) -> VibeResult<Self> {
        ...
        let backend = BackendFactory::create(
            &revibe_llm::backend::ProviderConfig {
        ...
        let messages = vec![LlmMessage::system(system_prompt)];
    ...
    async fn perform_llm_turn(
        ...
        if self.enable_streaming {
            ...
            // Batch content chunks before sending events (matching Python's BATCH_SIZE=5)
    ...
    pub fn load_history(&mut self, messages: Vec<LlmMessage>) -> VibeResult<()> {
        ...
        // Update stats from loaded messages
    ...
    async fn connect_mcp_servers(&mut self) -> VibeResult<()> {
        for server_config in &self.config.mcp_servers {
            let transport: Arc<dyn revibe_mcp::transport::Transport> = match server_config.transport
            {
