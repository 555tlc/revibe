...
//! between the ACP protocol and Revibe's agent.

use crate::types::{
    AgentMessageChunk, SessionUpdate, TextContentBlock, ToolCallContentVariant,
    ToolCallProgress, ToolCallStart, ToolKind,
};
use revibe_core::events::{ToolCallEvent, ToolResultEvent};
use revibe_core::utils::is_user_cancellation_event;
use serde_json::json;

/// Tool kind mapping
const TOOL_KIND: &[(&str, ToolKind)] = &[
...
pub fn tool_result_session_update(event: &ToolResultEvent) -> Option<SessionUpdate> {
    let (tool_status, raw_output) = if is_user_cancellation_event(event.skip_reason.as_deref()) {
        ...
        ("completed", event.result.as_ref().map(|r| r.to_string()).unwrap_or_default())
    } else {
        ("failed", event.error.as_ref().map(|e| e.to_string()).unwrap_or_default())
    };
